...
public class ConfigurationUiWiringTests
{
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"respectacle_test_{Guid.NewGuid():N}");
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"respectacle_save_test_{Guid.NewGuid():N}");
        try
        {
            Directory.CreateDirectory(tempDir);
    ...
    public void GetSuggestedFilename_DefaultSettings_UsesDefaultTemplate()
    {
        ...
        Assert.Contains("Respectacle_2026-05-01", filename);
    }
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"respectacle_multi_{Guid.NewGuid():N}");
        try
        {
            Directory.CreateDirectory(tempDir);
            var settings = new AppSettings { SaveLocation = tempDir };
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"respectacle_seq_{Guid.NewGuid():N}");
        try
        {
            var mediator = new ConfiguredExportFlowMediator(
                persistSettings: s =>
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"respectacle_cfg_{Guid.NewGuid():N}");
        try
        {
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"respectacle_cfg2_{Guid.NewGuid():N}");
        try
        {
