...
#[test]
fn captcho_capture_window_under_cursor_exists_with_correct_signature() {
    let _fn_ptr: extern "C" fn() -> CaptureResult = captcho_capture_window_under_cursor;
}
...
// ---------------------------------------------------------------------------
// Invalid HWND tests for captcho_capture_window_by_handle
// ---------------------------------------------------------------------------
...
fn capture_window_under_cursor_returns_valid_structure() {
    let result = captcho_capture_window_under_cursor();
    if result.status == CaptureStatus::Ok {
        assert!(!result.frame_data.is_null());
        assert!(result.width > 0);
        assert!(result.height > 0);
...
fn double_free_error_from_window_capture_is_safe() {
    ...
    unsafe {
        ...
        captcho_free_frame(result.frame_data); // double free null
