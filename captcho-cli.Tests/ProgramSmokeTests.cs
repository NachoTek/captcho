...
public class ProgramSmokeTests
{
    ...
    public void HelpArg_ReturnsHelpToStdout()
    {
        ...
        Assert.Equal(0, exitCode);
        Assert.Contains("captcho CLI", cap.Stdout);
    }
...
/// <summary>
/// Verifies the CLI project does NOT reference captcho-ui.
/// This ensures the headless CLI stays independent of the WinUI app.
/// </summary>
public class NoUiReferenceTests
{
    [Fact]
    public void CliProject_DoesNotReference_captchoUi()
    {
        string csproj = File.ReadAllText(
            Path.Combine(TestHelpers.ProjectRoot, "captcho-cli", "captcho-cli.csproj"));

        Assert.DoesNotContain("captcho-ui", csproj);
        Assert.DoesNotContain("captcho.UI", csproj);
    }

    [Fact]
    public void CliTestProject_DoesNotReference_captchoUi()
    {
        string csproj = File.ReadAllText(
            Path.Combine(TestHelpers.ProjectRoot, "captcho-cli.Tests", "captcho-cli.Tests.csproj"));
