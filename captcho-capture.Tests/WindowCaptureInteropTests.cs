...
public class WindowCaptureInteropTests
{
    ...
    {
        var method = typeof(NativeMethods).GetMethod("captcho_capture_window_under_cursor",
            BindingFlags.Public | BindingFlags.Static);
        Assert.NotNull(method);
        Assert.Equal(typeof(NativeCaptureResult), method!.ReturnType);
        Assert.Empty(method.GetParameters());
    ...
    {
        var names = new[] { "captcho_capture_active_window", "captcho_capture_window_under_cursor", "captcho_capture_window_by_handle" };
