...
// managed wrapper validation, and negative test cases for the captcho-capture library.
//
// These tests cover:
...
public class InteropContractTests
{
    ...
    {
        var method = typeof(NativeMethods).GetMethod("captcho_capture_monitor_by_index",
            BindingFlags.Public | BindingFlags.Static);
        Assert.NotNull(method);
        Assert.Equal(typeof(NativeCaptureResult), method!.ReturnType);
